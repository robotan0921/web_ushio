...
xlsx = Roo::Spreadsheet.open('./journal.xlsx')
...
for i in 2 .. xlsx.sheet(0).last_row
    ...
    string += " ``#{info[2]},''"                            # Title
    string += " #{info[3]},"                                # Journal
    string += " vol. #{info[4].to_i}," unless info[4].nil?  # Volume
    string += " no. #{info[5].to_i}," unless info[5].nil?   # Number
    string += " pp. #{info[6]}," unless info[6].nil?        # Pages
    string += " #{info[7]}" unless info[7].nil?             # Month
    string += " #{info[8].to_i}." unless info[8].nil?       # Year
    string += " DOI:#{info[9]}" unless info[9].nil?         # DOI
    string += "</li>\n\t\t\t"
    newlists << string
end
...
while line = f.gets 
    flag = true if line.include?('id="paper2016"')
    if flag
        if label
