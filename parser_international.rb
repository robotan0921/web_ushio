...
xlsx = Roo::Spreadsheet.open('./international.xlsx')
...
for i in 2 .. xlsx.sheet(0).last_row
    ...
    string += " pp. #{info[5]}," unless info[5].nil?        # Pages
    string += " #{info[6]}" unless info[6].nil?             # Location
...
while line = f.gets 
    flag = true if line.include?('id="international2016"')
    if flag
        if label
